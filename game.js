...
function preload() {
    game.load.image('starfield', 'assets/OGA-Background-2-Tiled.png');
    game.load.image('ship', 'assets/player.png');
    game.load.image('bullet', 'assets/bullet25.png');
...
function create() {
    ...
    starfield.scale.setTo(scaleRatio, scaleRatio);
   ...
   //music
    ...
    greenEnemies.scale.setTo(scaleRatio, scaleRatio);
    ...
    asteroid.createMultiple(3, 'asteroid');
    asteroid.setAll('anchor.x', 0.5);
    asteroid.setAll('anchor.y', 0.5);
    asteroid.setAll('scale.x', 0.5);
    asteroid.setAll('scale.y', 0.5);
    asteroid.setAll('angle', 180);
    asteroid.forEach(function(enemy){
        addEnemyEmitterTrail(enemy);
        enemy.body.setSize(enemy.width * 3 / 4, enemy.height * 3 / 4);
...
blueEnemyBullets.scale.setTo(scaleRatio, scaleRatio);
...
blueEnemies.scale.setTo(scaleRatio, scaleRatio);
...
boss.scale.setTo(scaleRatio, scaleRatio);
    boss.exists = false;
    boss.alive = false;
...
ray.scale.setTo(scaleRatio, scaleRatio);
        ray.alive = false;
        ray.visible = false;
        boss.addChild(ray);
...
ship.scale.setTo(scaleRatio, scaleRatio);
...
booster.scale.setTo(scaleRatio, scaleRatio);
    booster.width = 0;
...
shipTrail.scale.setTo(scaleRatio, scaleRatio);
    shipTrail.width = 10;
...
explosions.scale.setTo(scaleRatio, scaleRatio);
...
playerDeath.scale.setTo(scaleRatio, scaleRatio);
...
bossDeath.scale.setTo(scaleRatio, scaleRatio);
...
shields.scale.setTo(scaleRatio, scaleRatio);
...
gameOver.scale.setTo(scaleRatio, scaleRatio);
...
//asteroidLaunchFunction
function launchAsteroid(){
	var ASTEROID_SPEED = 200;

    var enemy = asteroid.getFirstExists(false);
    if (enemy) {
        enemy.reset(game.rnd.integerInRange(0, game.width), -20);
        enemy.body.velocity.x = game.rnd.integerInRange(-300, 300);
        enemy.body.velocity.y = ASTEROID_SPEED;
        enemy.body.drag.x = 100;

        enemy.trail.start(false, 800, 1);

        //  Update function for each asteroid to update rotation etc
        enemy.update = function(){
          enemy.angle = 180 - game.math.radToDeg(Math.atan2(enemy.body.velocity.x, enemy.body.velocity.y));
          ...
          //  Destroy asteroids once they go off screen
          if (enemy.y > game.height + 200) {
            enemy.kill();
            enemy.y = -20;
          }
    ...
    //  Send another asteroid soon
    asteroidLaunchTimer = game.time.events.add(game.rnd.integerInRange(asteroidSpacing, asteroidSpacing + 1000), launchAsteroid);
}
